...
{{ actresses }}さんが出演するこの作品。フェチ好きなら見逃せない一本です。
{% endif %}

{% if maker %}
> **{{ maker }}**からリリースされたこの作品は、フェチジャンルで定評があります。
{% endif %}
...
INTRO_VARIATIONS = [
    "フェチ心が止まらない...！**「{title}」**は{genre_text}好きにはたまらない一本です。",
    "フェチ好きにはたまらない...！**「{title}」**、{genre_text}好きなら絶対ハマる作品が来ました。",
    "{genre_text}で興奮したいならコレ！**「{title}」**がマジでおすすめ。",
    "話題沸騰中の**「{title}」**をピックアップ。{genre_text}のド直球フェチ作品です。",
    "**「{title}」**が気になってる人、正解です。{genre_text}ジャンルの中でもガチで抜けるフェチ作品。",
    "本日の厳選フェチAVは**「{title}」**。{genre_text}好きの間で「神作品」と話題に。",
    "新着から見つけた掘り出し物！**「{title}」**、{genre_text}がたっぷり詰まったフェチの極みです。",
    "サンプル動画だけでも興奮必至！**「{title}」**は{genre_text}の最高傑作かも。",
    "今週一番シコれるフェチ作品はコレ。**「{title}」**、{genre_text}好きは見逃すな！",
    "MuscleLoveが厳選！**「{title}」**は{genre_text}好きなら見逃せないフェチ作品です。",
    "MuscleLove編集部おすすめの**「{title}」**。{genre_text}ジャンルで今一番アツいフェチ作品。",
    "MuscleLoveイチオシ！**「{title}」**、{genre_text}好きを唸らせる最高のフェチ作品が来ました。",
]
...
def _generate_single_article(
    ...
    # タグの生成
    tag_list = genres[:5] if genres else ["フェチ", "乳首"]
    tags = ", ".join(f'"{t}"' for t in tag_list)

    # ジャンルテキスト（導入文用）
    genre_text = "・".join(genres[:3]) if genres else "フェチ"
...
def _build_meta_description(title: str, genre_text: str, actresses: str) -> str:
    ...
    desc_variations = [
        f"{title}のサンプル動画・レビュー。フェチ系の{genre_text}系作品を紹介。",
        f"フェチ好き必見の「{title}」を徹底レビュー。{genre_text}好きにおすすめのフェチAV。",
        f"フェチ心直撃！{genre_text}作品「{title}」。フェチシチュエーションに興奮必至。",
        f"フェチ作品の注目作「{title}」。{genre_text}のフェチシーンをサンプル動画付きで紹介。",
    ]
    if actresses:
        desc_variations.append(
            f"{actresses}出演「{title}」。{genre_text}系フェチ動画をチェック。"
        )
...
def _build_alt_text(title: str, actresses: str, genre_text: str) -> str:
    ...
    alt_variations = [
        f"フェチ作品「{title}」のパッケージ画像",
        f"「{title}」{genre_text}系フェチ作品のサムネイル",
        f"フェチ心直撃「{title}」の作品画像",
    ]
    if actresses:
        alt_variations.append(f"{actresses}出演「{title}」フェチ作品の画像")
...
"""
    for idx, img_url in enumerate(images, 1):
        gallery_html += f'  <img src="{img_url}" alt="フェチ作品のサンプル画像{idx}" style="width: 100%; border-radius: 4px;" loading="lazy" />\n'
